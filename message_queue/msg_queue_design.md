...
|------|--------|----------|----------|----------|----------|----------|--------|----------|
| 1. 单线程Queue | `single_thread_queue.h` | Single Thread | 无限制 | N/A | std::queue | 无 | ⭐ | 最高 |
| 2. 单线程循环队列 | `ring_buffer_queue.h` | Single Thread | 有界 | N/A | 环形缓冲区 | 无 | ⭐ | 最高 |
| 3. SPSC非阻塞 | `spsc_queue.h` | SPSC | 有界 | 非阻塞 | 环形缓冲区 | 原子操作 | ⭐⭐ | 极高 |
| 4. SPSC阻塞 | `spsc_blocking_queue.h` | SPSC | 有界 | 阻塞 | 环形缓冲区 | 原子操作+条件变量 | ⭐⭐ | 高 |
| 5. SPSC混合 | `spsc_timeout_queue.h` | SPSC | 有界 | 可选超时 | 环形缓冲区 | 原子操作+条件变量 | ⭐⭐⭐ | 高 |
| 6. MPMC简单 | `mpmc_simple_queue.h` | MPMC | 有界 | 阻塞 | std::queue | 互斥锁+条件变量 | ⭐⭐ | 中等 |
| 7. MPMC环形 | `mpmc_ring_queue.h` | MPMC | 有界 | 阻塞 | 环形缓冲区 | 互斥锁+条件变量 | ⭐⭐⭐ | 中等 |
| 8. MPMC无界 | `mpmc_unbounded_queue.h` | MPMC | 无限制 | 阻塞 | std::queue | 互斥锁+条件变量 | ⭐⭐ | 中等 |
| 9. 无锁MPMC | `lockfree_mpmc_queue.h` | MPMC | 无限制 | 非阻塞 | 链表 | CAS操作 | ⭐⭐⭐⭐⭐ | 极高 |
...
### 4. SPSC阻塞 (`spsc_blocking_queue.h`)
**设计特点**：
- 基础：SPSC非阻塞 + 条件变量
- 同步：一个mutex（仅用于条件变量）
- 等待：队列满/空时线程休眠
...
### 8. MPMC无界 (`mpmc_unbounded_queue.h`)
**设计特点**：
- 容量：无限制（仅受内存限制）
- 优势：永不阻塞生产者
- 风险：内存可能无限增长
...
### 9. 无锁MPMC (`lockfree_mpmc_queue.h`)
**设计特点**：
